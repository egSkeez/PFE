 ...
 */
public class UserGroupLdapAuthoritiesPopulator extends
		DefaultLdapAuthoritiesPopulator implements LdapAuthoritiesPopulator {

	private final Log logger = LogFactory
			.getLog(UserGroupLdapAuthoritiesPopulator.class);
	...
	public UserGroupLdapAuthoritiesPopulator(ContextSource contextSource,
	...
	/**
	 * 
	 * This function returns a list of roles from the given set of groups based
	 * on the value of the <code>groupToRoleMap</code> property.
	 * 
	...
	protected Set<GrantedAuthority> getAdditionalRoles(
		...
		if (logger.isDebugEnabled()) {
			logger.debug("Searching for roles for user '" + username
					...
					+ this.groupSearchFilter + " in search base '"
					+ super.getGroupSearchBase() + "'");
		}

		final Set<String> userRoles = ldapTemplate
				.searchForSingleAttributeValues(super.getGroupSearchBase(),
		...
		for (String group : userRoles) {
			...
			if (rolesForGroup != null) {
				for (String role : rolesForGroup) {
					...
					logger.debug("Added role: " + role + " based on group "
							+ group);
				}
			}
		}
